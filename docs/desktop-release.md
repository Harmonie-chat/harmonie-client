...
Then copy the private key content exactly:

```sh
cat ~/.tauri/harmonie.key
```

Add these GitHub Actions secrets:

- `TAURI_SIGNING_PRIVATE_KEY`: the exact output of `cat ~/.tauri/harmonie.key`, without quotes and without `TAURI_SIGNING_PRIVATE_KEY=`.
...
If the release fails with `failed to decode base64 secret key`, recreate `TAURI_SIGNING_PRIVATE_KEY`: it usually means the public key, an environment assignment, quotes, or extra text was pasted instead of the raw private key content.
