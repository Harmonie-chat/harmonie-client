...
describe('channels api', () => {
  ...
  it('sends messages with null content when empty', async () => {
    ...
    expect(apiFetchMock).toHaveBeenCalledWith(
      'https://localhost:5000/api/channels/channel-1/messages',
      {
        method: 'POST',
  ...
  it('calls void channel endpoints and reaction endpoints', async () => {
    ...
    expect(apiFetchMock).toHaveBeenCalledWith(
      'https://localhost:5000/api/channels/channel-1/messages/message-1/reactions/%F0%9F%91%8D',
      { method: 'PUT' }
    );
    expect(apiFetchMock).toHaveBeenCalledWith(
      'https://localhost:5000/api/channels/channel-1/messages/message-1/attachments/attachment%20%2F%201',
      { method: 'DELETE' }
    );
    expect(apiFetchMock).toHaveBeenCalledWith(
      'https://localhost:5000/api/channels/channel-1/voice/join',
      {
        method: 'POST',
      }
    );
